...
const BmiCalculator = () => {
  const parsedHeight = JSON.parse(localStorage.getItem('height'))
  const parsedWeight = JSON.parse(localStorage.getItem('weight'))

  const [height, setHeight] = useState(
    parsedHeight !== null ? parsedHeight : 170,
  )
  const [weight, setWeight] = useState(
    parsedWeight !== null ? parsedWeight : 60,
  )
